...
GREETING_MESSAGE = "220 classroom.cs.unc.edu"
...
def smtp_automaton(server_socket):
    '''
...
######################## HW 4 ############################
##########################################################
def parse_HELO_command(command, server_socket):
    '''
    check whether the given command is a valid HELO command
    '''
    if command[0:5] != 'HELO ':
        server_socket.close()
        start_listening(server_socket)
    if parse_domain(command, 5) == 0:
        server_socket.close()
        start_listening(server_socket)
    if command[-1] != '\n':
        server_socket.close()
        start_listening(server_socket)
    server_socket.send((command.strip('\n') + 'pleased to meet you\n').encode())

def greet(server_socket):
    '''
    After receiving the connection, send the Greting message then
    return True if recieved a valid SMTP HELP
    return false otherwise
    '''
    # recieve the connection and send the greeting message
    connection_socket, addr = server_socket.accept()
    connection_socket.send(GREETING_MESSAGE.encode())
...
def start_server():
    '''
    the main function to start the server process
    '''
